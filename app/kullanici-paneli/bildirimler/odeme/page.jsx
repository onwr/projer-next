...
async function getPaymentNotifications() {
  const res = await fetch(`${process.env.NEXTAUTH_URL || ''}/api/user/notifications/payment`, {
    cache: 'no-store',
  });
  if (!res.ok) return [];
...
export default async function PaymentNotificationsPage() {
  ...
  return (
    <div>
      <h1 className="mb-4 text-xl font-semibold text-gray-900">Ödeme Bildirimleri</h1>
